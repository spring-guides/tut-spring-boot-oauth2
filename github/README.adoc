...
In principle, once we start adding authentication providers, we may need to be more careful about the data returned from the "/user" endpoint.
It turns out that Github and Facebook both have a "name" field in the same place in their user info, so there isn't any change in practice to our simple endpoint.
...
The client details here are registered with https://github.com/settings/developers[Github], also with the address `localhost:8080` (same as for Facebook).
