...
To make the link visible, we also need to switch off the security on the home page by adding a `SecurityFilterChain` bean:
...
Spring Boot attaches special meaning to a `SecurityFilterChain` bean in the class annotated with `@SpringBootApplication`:
